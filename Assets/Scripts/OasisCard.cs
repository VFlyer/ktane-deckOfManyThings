...
class OasisCard : StandardCard {
    ...
    {
		return string.Format("Card Nº {0} is an Oasis {1} of {2} (Real Value: {3} of {4}).",
			order + 1, GetRank(fakeRank), GetFakeSuit(fakeSuit), GetRank(rank), GetSuit(suit));
    }
    public override string PrintTPCardInfo()
    {
		return string.Format("Card Nº {0} is an Oasis {1} of {2}.",
			order + 1, GetRank(fakeRank), GetFakeSuit(fakeSuit));
	}
