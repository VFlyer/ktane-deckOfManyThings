...
class CelestialCard : StandardCard {
    ...
    {
		return string.Format("Card Nº {0} is a Celestial card: {1}.", order + 1, GetArcana(arcana));
    }
	...
	public List<int> GetValidSuits(KMBombInfo bomb, List<int> suits, int startTime, string date, StandardCard firstCard)
	{
		switch(arcana)
		{
			...
			case 17:
			{
				...
				{
					if(firstCard.rank >= 2 && firstCard.rank <= 6) ret.AddRange(new int[] {SPADES});
					else if(firstCard.rank >= 7 && firstCard.rank <= 9) ret.AddRange(new int[] {CLUBS});
					else ret.AddRange(new int[] {DIAMONDS, HEARTS});
				}
				else if(firstCard.GetType() == typeof(MetropolitanCard))
				{
					if(firstCard.rank >= 2 && firstCard.rank <= 6) ret.AddRange(new int[] {SPADES, CLUBS});
					else if(firstCard.rank >= 7 && firstCard.rank <= 9) ret.AddRange(new int[] {SPADES});
					else ret.AddRange(new int[] {CLUBS});
				}
				...
				{
					if(firstCard.rank >= 2 && firstCard.rank <= 6) ret.AddRange(new int[] {SPADES});
					else if(firstCard.rank >= 7 && firstCard.rank <= 9) ret.AddRange(new int[] {SPADES, CLUBS});
					else ret.AddRange(new int[] {HEARTS, DIAMONDS});
				}
				else if(firstCard.GetType() == typeof(TropicalCard))
				{
					if(firstCard.rank >= 2 && firstCard.rank <= 6) ret.AddRange(new int[] {HEARTS, DIAMONDS});
					else if(firstCard.rank >= 7 && firstCard.rank <= 9) ret.AddRange(new int[] {SPADES, CLUBS});
					else ret.AddRange(new int[] {CLUBS});
				}
